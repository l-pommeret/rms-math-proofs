...
/-! ## Gate 3 : the two-term stationary phase expansion of `f 2` on the negative axis -/

/-- **Gate 3.**  The two-term stationary phase expansion of `f 2 (-R²)`, with an explicit
uniform additive remainder. -/
theorem f_two_stationary_phase :
    ∃ C R0 : ℝ, 0 ≤ C ∧ 1 ≤ R0 ∧
      ∀ R : ℝ, R0 ≤ R →
        ‖f 2 (-((R:ℂ)^2))
          - ((1 / Real.sqrt (π*R)
                * (Real.cos (2*R - π/4)
                    + (1/(16*R)) * Real.sin (2*R - π/4)) : ℝ) : ℂ)‖
          ≤ C * (1 / Real.sqrt (π*R)) / R^2 := by
  obtain ⟨C, hC0, hC⟩ := besselJ0_expansion
  refine ⟨C, 1, hC0, le_refl _, fun R hR => ?_⟩
  have hpi := Real.pi_pos
  have hR0 : (0:ℝ) < R := by linarith
  have hb := hC (2*R) (by linarith)
  have hsq : Real.sqrt (2/(π*(2*R))) = 1 / Real.sqrt (π*R) := by
    rw [show 2/(π*(2*R)) = (π*R)⁻¹ by field_simp, Real.sqrt_inv, one_div]
  have hsin : Real.sin (2*R - π/4)/(8*(2*R)) = (1/(16*R)) * Real.sin (2*R - π/4) := by
    field_simp; ring
  rw [hsq, hsin] at hb
  rw [f_two_neg hR0.le]
  rw [show ((besselJ0 (2*R) : ℝ) : ℂ)
      - ((1 / Real.sqrt (π*R) * (Real.cos (2*R - π/4)
          + (1/(16*R)) * Real.sin (2*R - π/4)) : ℝ) : ℂ)
      = ((besselJ0 (2*R) - 1 / Real.sqrt (π*R) * (Real.cos (2*R - π/4)
          + (1/(16*R)) * Real.sin (2*R - π/4)) : ℝ) : ℂ) by push_cast; ring]
  rw [Complex.norm_real, Real.norm_eq_abs]
  refine le_trans hb ?_
  have hsR : (0:ℝ) < Real.sqrt R := Real.sqrt_pos.2 hR0
  have hspiR : (0:ℝ) < Real.sqrt (π*R) := Real.sqrt_pos.2 (by positivity)
  have hform : C * (1 / Real.sqrt (π*R)) / R^2 = C / (Real.sqrt (π*R) * R^2) := by field_simp
  rw [hform]
  refine div_le_div_of_nonneg_left hC0 (by positivity) ?_
  have hfac : Real.sqrt (π*R) = Real.sqrt π * Real.sqrt R := Real.sqrt_mul hpi.le R
  have hfac2 : Real.sqrt (2*R) = Real.sqrt 2 * Real.sqrt R := Real.sqrt_mul (by norm_num) R
  have hpi2 : Real.sqrt π ≤ 2 := by
    nlinarith [Real.sq_sqrt hpi.le, Real.sqrt_nonneg π, Real.pi_le_four]
  have h2 : (1:ℝ) ≤ Real.sqrt 2 := by
    nlinarith [Real.sq_sqrt (show (0:ℝ) ≤ 2 by norm_num), Real.sqrt_nonneg 2]
  rw [hfac, hfac2]
  have hA : Real.sqrt π * Real.sqrt R * R^2 ≤ 2 * (Real.sqrt R * R^2) := by
    nlinarith [Real.sqrt_nonneg R, sq_nonneg R, mul_nonneg (Real.sqrt_nonneg R) (sq_nonneg R)]
  have hB : 2 * (Real.sqrt R * R^2) ≤ (2*R)^2 * (Real.sqrt 2 * Real.sqrt R) := by
    nlinarith [mul_nonneg (Real.sqrt_nonneg R) (sq_nonneg R), Real.sqrt_nonneg R, sq_nonneg R]
  linarith

/-- Gate 2 in the case `m = 2`, phrased with `amplitude`, `phase` and `firstCoeff`. -/
theorem two_term_additive_two :
    ∃ C R0 : ℝ, 0 ≤ C ∧ 1 ≤ R0 ∧
      ∀ R : ℝ, R0 ≤ R →
        ‖f 2 (-((R:ℂ)^2))
          - ((amplitude 2 R * (Real.cos (phase 2 R)
              + firstCoeff 2 / R * Real.cos (phase 2 R - 1*π/2)) : ℝ) : ℂ)‖
          ≤ C * amplitude 2 R / R^2 := by
  obtain ⟨C, R0, hC0, hR0, h⟩ := f_two_stationary_phase
  refine ⟨C, R0, hC0, hR0, fun R hR => ?_⟩
  have hRpos : (0:ℝ) < R := by linarith
  have hA : amplitude 2 R = 1 / Real.sqrt (π*R) := amplitude_two hRpos
  have hmain : amplitude 2 R * (Real.cos (phase 2 R)
      + firstCoeff 2 / R * Real.cos (phase 2 R - 1*π/2))
      = 1 / Real.sqrt (π*R) * (Real.cos (2*R - π/4) + (1/(16*R)) * Real.sin (2*R - π/4)) := by
    rw [hA, cos_phase_sub_pi_div_two, firstCoeff_two, phase_two]
    ring
  rw [hmain, hA]
  exact h R hR

/-- The leading additive estimate in the case `m = 2`. -/
theorem leading_additive_two :
    ∃ C R0 : ℝ, 0 ≤ C ∧ 1 ≤ R0 ∧
      ∀ R : ℝ, R0 ≤ R →
        ‖f 2 (-((R:ℂ)^2)) - ((amplitude 2 R * Real.cos (phase 2 R) : ℝ) : ℂ)‖
          ≤ C * amplitude 2 R / R := by
  obtain ⟨C, R0, hC0, hR0, h⟩ := two_term_additive_two
  refine ⟨C + 1/16, R0, by linarith, hR0, fun R hR => ?_⟩
  have hRpos : (0:ℝ) < R := by linarith
  have hR1 : (1:ℝ) ≤ R := by linarith
  have hAnn : 0 ≤ amplitude 2 R := amplitude_nonneg (by norm_num) hRpos
  have hsplit : f 2 (-((R:ℂ)^2)) - ((amplitude 2 R * Real.cos (phase 2 R) : ℝ) : ℂ)
      = (f 2 (-((R:ℂ)^2)) - ((amplitude 2 R * (Real.cos (phase 2 R)
              + firstCoeff 2 / R * Real.cos (phase 2 R - 1*π/2)) : ℝ) : ℂ))
        + ((amplitude 2 R * (firstCoeff 2 / R * Real.cos (phase 2 R - 1*π/2)) : ℝ) : ℂ) := by
    push_cast; ring
  rw [hsplit]
  refine le_trans (norm_add_le _ _) ?_
  have hb2 : ‖((amplitude 2 R * (firstCoeff 2 / R * Real.cos (phase 2 R - 1*π/2)) : ℝ) : ℂ)‖
      ≤ amplitude 2 R * (1/16) / R := by
    rw [Complex.norm_real, Real.norm_eq_abs, abs_mul, abs_of_nonneg hAnn, firstCoeff_two]
    have hc : |1/16/R * Real.cos (phase 2 R - 1*π/2)| ≤ 1/16/R := by
      rw [abs_mul, abs_of_nonneg (by positivity : (0:ℝ) ≤ 1/16/R)]
      nlinarith [Real.abs_cos_le_one (phase 2 R - 1*π/2),
        abs_nonneg (Real.cos (phase 2 R - 1*π/2)), (by positivity : (0:ℝ) ≤ 1/16/R)]
    calc amplitude 2 R * |1/16/R * Real.cos (phase 2 R - 1*π/2)|
        ≤ amplitude 2 R * (1/16/R) := mul_le_mul_of_nonneg_left hc hAnn
      _ = amplitude 2 R * (1/16) / R := by ring
  refine le_trans (add_le_add (h R hR) hb2) ?_
  have he : (C + 1/16) * amplitude 2 R / R
      = C * amplitude 2 R / R + amplitude 2 R * (1/16) / R := by ring
  rw [he]
  have hle : C * amplitude 2 R / R^2 ≤ C * amplitude 2 R / R :=
    div_le_div_of_nonneg_left (by positivity) hRpos (by nlinarith)
  linarith
